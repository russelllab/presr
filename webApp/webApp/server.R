...
shinyServer(
  ...
  observeEvent(input$outputhelp, {
    ...
    observeEvent(input$outputHelp, {
      showModal(modalDialog(
        title = "Output Help",
        HTML("<div style='font-size: 15px'>Predicted probabilities of wild type and variant to be disease.<br><kbd>Type</kbd>: Wild type or the variant<br><kbd>AA</kbd>: Amino acid<br><kbd>PROB</kbd>: PRESR predicted probability to be a disease variant (neutral is <0.5, otherwise disease)</div>"
        )
      ))
    })
